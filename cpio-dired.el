...
(defun OBS-cpio-dired-get-marked-entries (&optional arg filter distinguish-one-marked)
...
Marks win over ARG."
  ...
  (let ((fname "cpio-dired-do-delete")
    ...
    (mapc (lambda (en)
	    (save-excursion
	      (goto-char (point-min))
	      ;; This search is guaranteed to succeed because of the construction of ENTRIES.
